...
library(QRegVCM)
data(CD4)
data_obs = CD4%>%dplyr::select(subj,Time, CD4)
observed = lapply(unique(data_obs$subj), function(x) {
	t  = data_obs%>%dplyr::filter(subj==x)%>%with(Time)
	y = data_obs%>%dplyr::filter(subj==x)%>%with(CD4)
	y = y[!duplicated(t)]
	y
})

timepoints = lapply(unique(data_obs$subj), function(x) {
	t  = data_obs%>%dplyr::filter(subj==x)%>%with(Time)
	t = t[!duplicated(t)]
	t
})
...
################SOAP###################
########################################
spline_basis=create.bspline.basis(rangeval=c(0,6),nbasis=10,norder=4)
